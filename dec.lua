...
function LuauDecompiler:DecompileInstruction(instr, proto, pc)
    local op = INSN_OP(instr)
    local opname = OpCodes[op] or ("UNKNOWN_" .. op)
    local A = INSN_A(instr)
    local B = INSN_B(instr)
    local C = INSN_C(instr)
    local D = INSN_D(instr)
    
    local output = ""
    
    if opname == "LOADNIL" then
        output = string.format("R%d = nil", A)
    elseif opname == "LOADB" then
        output = string.format("R%d = %s", A, B ~= 0 and "true" or "false")
    elseif opname == "LOADN" then
        output = string.format("R%d = %d", A, D)
    elseif opname == "LOADK" then
        local const = proto.constants[D + 1]
        if type(const) == "string" then
            output = string.format('R%d = "%s"', A, const:gsub('"', '\\"'))
        elseif type(const) == "number" then
            output = string.format("R%d = %s", A, tostring(const))
        else
            output = string.format("R%d = %s", A, tostring(const))
        end
    elseif opname == "MOVE" then
        output = string.format("R%d = R%d", A, B)
    elseif opname == "GETGLOBAL" then
        -- The constant index follows in aux
        output = string.format("R%d = _G[...]", A)
    elseif opname == "SETGLOBAL" then
        output = string.format("_G[...] = R%d", A)
    elseif opname == "CALL" then
        output = string.format("R%d(%d args, %d returns)", A, B - 1, C - 1)
    elseif opname == "RETURN" then
        if B == 0 then
            output = "return"
        else
            output = string.format("return R%d..R%d", A, A + B - 2)
        end
    elseif opname == "JUMP" then
        local offset = D - 32768 -- Sign extend
        output = string.format("jump %+d", offset)
    elseif opname == "JUMPIF" then
        local offset = D - 32768
        output = string.format("if R%d then jump %+d", A, offset)
    elseif opname == "JUMPIFNOT" then
        local offset = D - 32768
        output = string.format("if not R%d then jump %+d", A, offset)
    else
        output = string.format("%s (A:%d B:%d C:%d D:%d)", opname, A, B, C, D)
    end
    
    return string.format("[%04d] %s", pc, output)
end
...
function LuauDecompiler:Decompile(proto, bytecodeInfo, level)
    ...
    for pc, instr in ipairs(proto.instructions) do
        local decompiledInstr = self:DecompileInstruction(instr, proto, pc - 1)
        table.insert(output, indent .. decompiledInstr)
    end
...
function decompilev2(input)
    ...
    print("🚀 Universal Luau Decompiler V2")
